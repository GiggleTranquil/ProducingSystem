...
class MAIN_ui(QtWidgets.QMainWindow,MAIN.Ui_MainWindow):
    def __init__(self):
        ...
        self.pushButton_3.clicked.connect(self.reverse_inference)
    ...
    def reverse_inference(self):
        # 反向推理
        conclusion = self.textEdit.toPlainText()  # 获取结论
        DB = set(conclusion.split('\n'))  # 获取输入的事实，以换行符分割，并转换成集合
        [P, Q] = get_rules()  # 获取规则库
        self.process = ''  # 用于存储推理过程
        self.animal = ''  # 存储结论
        # 下面开始正式推理
        flag = 0
        for conclusion in Q:  # 对前提条件进行遍历
            if ListInSet(conclusion, DB):
                # 能够找到一个前提条件全部存在于数据库
                self.process += "所有前提条件：" + str(P[Q.index(conclusion)]) + "\n"
                self.animal = str(P[Q.index(conclusion)])
                flag = 1  # 至少有一个能够推出来的结论
        if flag == 0:
            # 一个结论也推不出来，询问用户是否进行补充
            self.alert_window.show()
